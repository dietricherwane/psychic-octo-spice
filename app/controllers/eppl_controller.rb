...
#  before_filter :check_ip, only: [:charge_eppl_account, :api_place_bet, :api_pay_earning, :api_transfer_earning, :api_recharge_eppl_account]
  ...
  def check_ip
    ...
    if !(['149.202.202.26', '94.247.179.9', '172.18.2.12', ' 192.168.1.41', '82.97.38.138', '41.21.163.46', '195.14.0.128'].include?(remote_ip_address) rescue false)
      render text: 'moron'
    end
  end
        ...
        request.on_complete do |response|
          if response.success?
            ...
            if !response_body.include?("|")
              ...
              Eppl.create(operation: "Chargement de compte", bet_validated: true, transaction_id: @transaction_id, paymoney_account_number: paymoney_account_number, transaction_amount: transaction_amount, game_account_token: game_account_token, bet_placed_at: DateTime.now, gamer_id: @gamer_id) rescue nil
            else
              case response_body
                when "|0|"
                  @error_code = '4001'
