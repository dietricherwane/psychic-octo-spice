      ...
      request.on_complete do |response|
        if response.success?
          ...
          if !json_response.blank?
            ...
            if @error_code == 0 && (json_response["header"]["status"] == 'success' rescue nil)
              bet_date = json_response["header"]["dateTime"] rescue nil
              @bet = (json_response["content"] rescue nil)
        ...
        if validate_bet_ail("AliXTtooY", bets_amount, "ail_pmus")
          ...
          unless bets_refund.blank?
            bets_refund.each do |bet_refund|
              ...
              # Email notification
              WinningNotification.notification_email(bet_refund.user, bet_refund.refund_amount, "au LOTO", "LOTO", bet_refund.ticket_number, bet_refund.paymoney_account_number).deliver
            end
          end
