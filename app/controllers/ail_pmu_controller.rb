class AilPmuController < ApplicationController
  ...
  def check_ip
    ...
    if !(['149.202.202.26', '41.189.40.193', '94.247.179.9', '172.18.2.12', ' 192.168.1.41', '82.97.38.138', '41.21.163.46', '195.14.0.128', '41.21.163.44'].include?(remote_ip_address) rescue false)
      render text: 'moron'
    end
  end
...
=end
          ...
          if !json_response.blank?
            ...
            if @error_code == 0 && (json_response["header"]["status"] == 'success')
              ...
              #unless tmp_bet_date.blank?
                ...
                #tmp_bet_date = tmp_date[1] + "/" + tmp_date[0] + "/" + tmp_date[2] + " " + tmp_bet_date[1] + " " + tmp_bet_date[2] rescue ""
              #end
  ...
  def api_gamer_bets
    ...
    if user.blank?
      ...
      @bets = user.ail_pmus.where("bet_status IS NOT NULL").order("created_at DESC") rescue nil
    end
  end
  ...
  def payment_notification_earning
    ...
    # Email notification
    WinningNotification.notification_email(@bet.user, @bet.earning_amount, "au PMU-PLR", "PMU-PLR", @bet.ticket_number, @bet.paymoney_account_number, @bet.ref_number, @bet).deliver
  end
  ...
  def payment_notification_refund
    ...
    # Email notification
    WinningNotification.notification_email(@bet.user, @bet.refund_amount, "au PMU-PLR", "PMU-PLR", @bet.ticket_number, @bet.paymoney_account_number, @bet.ref_number, @bet).deliver
  end
        ...
        if validate_bet_ail("ApXTrliOp", bets_amount, "ail_pmus")
          ...
          unless bets_payout.blank?
            bets_payout.each do |bet_payout|
              ...
              # Email notification
              WinningNotification.notification_email(bet_payout.user, bet_payout.earning_amount, "au PMU-PLR", "PMU-PLR", bet_payout.ticket_number, bet_payout.paymoney_account_number, bet_payout.ref_number, bet_payout).deliver
            end
          end
          ...
          unless bets_refund.blank?
            bets_refund.each do |bet_refund|
              ...
              # Email notification
              WinningNotification.notification_email(bet_refund.user, bet_refund.refund_amount, "au PMU-PLR", "PMU-PLR", bet_refund.ticket_number, bet_refund.paymoney_account_number, bet_refund.ref_number, bet_refund).deliver
            end
          end
          ...
          if validate_bet_ail("ApXTrliOp", bets_amount, "ail_pmus")
            ...
            unless bets_payout.blank?
              bets_payout.each do |bet_payout|
                ...
                # Email notification
                WinningNotification.notification_email(bet_payout.user, bet_payout.earning_amount, "au PMU PLR", "PMU PLR", bet_payout.ticket_number, bet_payout.paymoney_account_number, bet_payout).deliver
              end
            end
            ...
            unless bets_refund.blank?
              bets_refund.each do |bet_refund|
                ...
                # Email notification
                WinningNotification.notification_email(bet_refund.user, bet_refund.refund_amount, "au PMU PLR", "PMU PLR", bet_refund.ticket_number, bet_refund.paymoney_account_number, bet_refund).deliver
              end
            end
          end
